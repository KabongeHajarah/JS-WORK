...
const stringSet= new Set (['bella','shaturah','rayan','juliet','Maleek']);
console.log(stringSet);
// Create a map of countries and number of characters of a country
 countries=['America','German','India','Burundi','Uganda','Paris','Turkey','Netherlands']
const newCountriesMap=new Map()
for (const country of countries) {
    newCountriesMap.set(country, country.length)
}
console.log(newCountriesMap);


const countriesMaps= new Map()
countriesMaps.set('America',7)
countriesMaps.set('Uganda',6)
countriesMaps.set('Dubai',5)
console.log(countriesMaps);
...
const countriesN = [
  ...
  const setCountries=new Set(countriesN)
 function countLanguage(countriesN) {
    const languages=new Set();

    countriesN.forEach((country)=>{
country.languages.forEach((language)=>{
 ...
 const numberOfLanguages=countLanguage(countriesN);
 console.log(numberOfLanguages);
  // *** Use the countries data to find the 10 most spoken languages:
function mostSpoken(countriesN,limit) {
    const tenSpoken= new Map();
    
    countriesN.forEach((country)=>{
        country.languages.forEach((language)=>{
            if (tenSpoken.has(language)) {
                tenSpoken.set(language, tenSpoken.get(language) +1);
            }else{
                tenSpoken.set(language,1);
            }
        });
    });

    const sortedLanguages = [...tenSpoken.entries()].sort(
        (a, b) => b[1] - a[1]
      );
    
      const topLanguages = sortedLanguages.slice(0, limit).map((entry) => {
        const [language, count] = entry;
